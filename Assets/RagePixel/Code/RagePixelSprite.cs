...
public class RagePixelSprite : MonoBehaviour, IRagePixel {
            ...
            if (nextAnimFrame < myTime)
            {
                switch (animationMode)
                {
                    case(AnimationMode.PlayOnce):
                        if (GetCurrentRow().GetIndex(currentCellKey) < GetCurrentRow().cells.Length - 1 && animationMaxIndex < 0 || GetCurrentRow().GetIndex(currentCellKey) < animationMaxIndex)
                        {
                    ...
                    case (AnimationMode.PlayOnceReverse):
                        if (GetCurrentRow().GetIndex(currentCellKey) > Mathf.Max(animationMinIndex, 0))
                        {
                            while (nextAnimFrame < myTime)
                            {
                                shiftCell(-1, false);
    ...
    public void PlayAnimation(bool forceRestart, int rangeMinIndex, int rangeMaxIndex)
    {
        if (playAnimation == false || forceRestart)
        {
    ...
    public void CheckAnimRangesOnPlay()
    {
        switch (animationMode)
        {
            case (AnimationMode.PlayOnce):
    ...
    public void PlayNamedAnimation(string name, bool forceRestart, float delayFirstFrame)
    {
        RagePixelAnimation animation = GetCurrentRow().GetAnimationByName(name);
        if (animation != null)
        {
